...
private extension 🥽AppModel {
    private func processHandUpdates() async {
        for await update in self.handTracking.anchorUpdates {
            ...
            resultString.append("\n\nEntity update cycle at \(getTime())\nStarting Major Joint Update")
            ...
            for i in 0..<(majorHandJoints.count) {
                ...
                // Log position of each major joint
                resultString.append("\nMajor Joint #\(i + start) update to position \(worldPos)")
                print(worldPos)
            }
            // Minor joints now
            start = handAnchor.chirality == .left ? 0 : minorHandJoints.count
            
            resultString.append("\n\nStarting Minor Joint Update")
            for i in 0..<(minorHandJoints.count) {
                ...
                // Log position of each major joint
                resultString.append("\nMinor Joint #\(i + start) update to position \(worldPos)")
            }
            
            resultString.append("\nEntity update finished at \(getTime())")
        }
    }
}
